...
class BaseTemplateTest(TestCase):
    ...
    # Test if static files/external resources are included in the template
    def test_static_files_included(self):
        ...
        self.assertContains(
            response,
            'href="{}"'.format(static('css/style.css'))
        )

        # Check if the external JS files are included
        self.assertContains(
            response,
            'src="https://kit.fontawesome.com/44dbf25b5c.js"'
        )
        self.assertContains(
            response,
        ...
        self.assertContains(
            response,
            'src="{}"'.format(static('js/script.js'))
        )
    ...
    def test_template_inheritance(self):
        ...
        self.assertContains(
            response, '<footer class="container-fluid'
    ...
    def test_render_base_template(self):
        ...
        self.assertIn(
            ...
            'css/bootstrap.min.css" rel="stylesheet"', html_output
        )
        self.assertIn(
            '<a class="navbar-brand text-color-sec" href="/"'
            '>Barber Booking</a>', html_output
        )
        self.assertIn(
            '<footer class="container-fluid'
            'text-color-sec pt-4 fixed-bottom">', html_output
        )
